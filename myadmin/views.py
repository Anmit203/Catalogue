...
def order_report(request):
    if request.method =='POST':
        from_date = request.POST['from_date']
        to_date   = request.POST['to_date']
        result = Order.objects.filter(orderdate__gte=from_date,orderdate__lte=to_date)
        request.session['from_date'] = from_date
        request.session['to_date'] = to_date
        if result.exists():
...
class GenerateOrderPdf(View):
     def get(self, request, *args, **kwargs):
        from_date = request.session['from_date']
        to_date   = request.session['to_date']
        data = Order.objects.filter(orderdate__gte=from_date,orderdate__lte=to_date)
        cdate = date.today()
        cdate1 = cdate.strftime('%d/%m/%Y')
...
def store_product(request):
    ...
    Product.objects.create(pname=pname,price=price,quantity=quantity,company=author,description=description,image=image.name,size=size,category_id=category_id,subcategory_id=subcategory_id)
...
def all_products(request):
    ...
    context = {'result':result}
    return render(request, 'myadmin/all_products.html',context)
...
def delete_product(request, id):
    ...
    return redirect('/myadmin/all_products')
