...
def store_product(request):
    ...
    Product.objects.create(pname=pname,price=price,quantity=quantity,company=author,description=description,image=image.name,size=size,category_id=category_id,subcategory_id=subcategory_id,seller_id=result.id)
...
def pending_order(request):
    ...
    result = Order.objects.filter(seller_id=seller.id)
    context = {'result':result}
    return render(request, 'seller/pending_order.html',context)
...
def change_status(request, id):
    ...
    context = {'result':result}
    return render(request, 'seller/change_status.html',context)

def update_status(request, id):
    status = request.POST['status']

    if status == 'reschedule':
        return render(request, 'seller/reschedule.html', {'id1':id})

    elif status == 'visited':
        return render(request, 'seller/visited.html', {'id1':id})
    ...
    return redirect('/seller/pending_order')
...
def order_reschedule(request, id):
    ...
    result = Order.objects.get(pk=id)
    # r = result.id
    result2 = Order_details.objects.get(order_id=result.id)
    oo = Order.objects.create(amount=result.amount,ddate=ddate,status='reschedule',customer_id=result.customer_id,seller_id=result.seller_id,area_id=result.area_id,city_id=result.city_id,fname=result.fname,lname=result.lname,phone=result.phone,time=time,orderdate=result.orderdate)
    Order_details.objects.create(quantity=result2.quantity,price=result2.price,product_id=result2.product_id,order_id=oo.id,seller_id=result2.seller_id)
    Order.objects.update_or_create(pk=id, defaults=data)
    return redirect('/seller/pending_order')

def update_visited(request,id):
    description = request.POST['description']
    data = {
                'description':description,
                'status' : 'visited'
...
def order_report(request):
    ...
    if request.method =='POST':
        ...
        result = Order.objects.filter(ddate__gte=from_date,ddate__lte=to_date,seller_id=s_id)
...
class GenerateOrderPdf(View):
     def get(self, request, *args, **kwargs):
        ...
        data = Order.objects.filter(ddate__gte=from_date,ddate__lte=to_date,seller_id=s_id)
        cdate = date.today()
